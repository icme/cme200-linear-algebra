...
We know from the lectures that $N(U) = N(A)$. For completeness, this can be proven as follows (but not necessary to do this on the midterm solutions):
First, we know that $N(U) \subseteq N(A)$ since if $\vec x \in N(U)$, then  $U \vec x = \vec 0$ and $A \vec x = LU\vec x = L \vec 0 = \vec 0$, so $\vec x \in N(A)$. Then, we know that  $L$ is nonsingular as it is a triangular matrix with non-zero entries on the diagonal. In other words, $L$ has full rank. Because $A = LU$, we then know that  $N(A) \subseteq N(U)$. We can also convince ourselves of the latter in the following way: suppose that we have a  $\vec x \in N(A)$ and $\vec x \not\in N(U)$, then $U\vec x = \vec y \ne \vec 0$ and we get a contradiction $A \vec x = L U \vec x = L \vec y \ne \vec 0$, since $L$ has full column rank.\\

Using that $N(A) = N(U)$, we can find a basis for the null space of $A$ by working with $U$ directly.
\end{enumerate}
}
...
\problemAnswer{
Unfortunately, many people made mistakes in computing the inverse of $L$. The most common mistake was to take $L$ and simply
multiply all off-diagonal elements by $-1$. Careful though! This could be done for the elementary building blocks of $L$ (the $C_i$'s introduced
in class) but it can \emph{not} by done on $L$ itself. You can easily convince yourself by multiplying the matrix you would get in this
case with $L$. You will not get the identity. \\ \\
...
The crux here is to recognize that is just says the vectors \emph{span} then subspace. It does not say the vectors form a basis. The vectors form a spanning set and this spanning set can be redundant.
...
\item
%%%%% 3b     %%%%%%%%%
Here, we use the perturbation result we derived in class:
\[
\frac{\| \vec{\delta x} \|}{\|x\|} \leq \kappa(A) \frac{ \| \vec{\delta b}\|}{\| \vec{b} \|},
\]
We know that $\kappa(A) = \| A \| \|A^{-1}\|$. We will use the Frobenius norm in the evaluation.
Clearly $\|A\|_F = \sqrt{2^2 + 1^2 + 2^2 + 1^2   } = \sqrt{10}$. We also see that 
\[
A^{-1} = \frac{1}{3} \left[ \begin{array}{cc} 2 & -1 \\ -1 & 2 \end{array} \right],
\]
giving $\|A^{-1} \|_F = \frac{\sqrt{10}}{3}$, and so $\kappa(A) = 10/3$.
Further we have that $\| \vec{x} \|_2 = \sqrt{2}, \| \vec{b} \|_2 = 3 \sqrt{2}, \| \vec{\delta b} \|_2 = 0.001$.
This gives $\| \vec{ \delta x} \|_2 \leq \frac{10}{3} \sqrt{2} \frac{0.001}{3 \sqrt{2}} = 0.01/9$.

Not everybody remembered this formula. A common mistake was that the formula was used
not including norms, or with only some of the components including norms. 
